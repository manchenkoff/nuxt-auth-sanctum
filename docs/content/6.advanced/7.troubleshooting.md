...
### Cookies are not set on the first page load with SSR streaming enabled

When you enable `experimental.ssrStreaming`, the module may log the following warning:
`[response] response headers are already sent, cookies will be replayed on the client`.

This happens because the response headers are committed with the HTML shell before the page 
finishes rendering, so cookies received during SSR cannot be attached to the response anymore. 
The module handles this by replaying non-HttpOnly cookies to the browser after hydration.

If you see this warning and the session (HttpOnly) cookie is missing after the first load, 
it means your CSRF endpoint also sets an HttpOnly session cookie that cannot be written by JavaScript. 
The module re-requests the CSRF cookie after hydration to make the browser store it natively, 
but if this does not help, move the session cookie creation to the login endpoint so it is 
only set during the authenticated login flow.
